...
export class TilingManager {
    ...
    public enable() {
        this._signals.connect(
            ...
            () => {
                const ws = (global.workspace_manager ?? global.workspaceManager).get_active_workspace();
                if (!ws) return;
    ...
    private _onSnapAssist(_: SnapAssist, tile: Tile, layoutId: string) {
        ...
        const currentWs = (global.workspace_manager ?? global.workspaceManager).get_active_workspace();
        const tilingLayout = this._workspaceTilingLayout.get(currentWs);
