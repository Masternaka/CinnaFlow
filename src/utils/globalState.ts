...
export default class GlobalState extends GObject.Object {
    ...
    constructor() {
        ...
        this._signals.connect(
            ...
            () => {
                ...
                const n_workspaces = (global.workspace_manager ?? global.workspaceManager).get_n_workspaces();
                for (let i = 0; i < n_workspaces; i++) {
                    const ws =
                        (global.workspace_manager ?? global.workspaceManager).get_workspace_by_index(i);
                    if (!ws) continue;
        ...
        this._signals.connect(
            (global.workspace_manager ?? global.workspaceManager),
            'workspace-added',
            (_, index: number) => {
                const n_workspaces = (global.workspace_manager ?? global.workspaceManager).get_n_workspaces();
                const newWs =
                    (global.workspace_manager ?? global.workspaceManager).get_workspace_by_index(index);
                if (!newWs) return;

                debug(`added workspace ${index}`);

                const secondLastWs =
                    (global.workspace_manager ?? global.workspaceManager).get_workspace_by_index(
                        n_workspaces - 2,
                    );
    ...
    public validate_selected_layouts() {
        ...
        for (let i = 0; i < (global.workspace_manager ?? global.workspaceManager).get_n_workspaces(); i++) {
            const ws = (global.workspace_manager ?? global.workspaceManager).get_workspace_by_index(i);
            if (!ws) continue;
    ...
    public setSelectedLayoutOfMonitor(
        ...
        selected[(global.workspace_manager ?? global.workspaceManager).get_active_workspace_index()][
            monitorIndex
        ] = layoutToSelectId;
        ...
        const n_workspaces = (global.workspace_manager ?? global.workspaceManager).get_n_workspaces();
        if (
            (global.workspace_manager ?? global.workspaceManager).get_active_workspace_index() ===
            n_workspaces - 2
        ) {
            const lastWs = (global.workspace_manager ?? global.workspaceManager).get_workspace_by_index(
                n_workspaces - 1,
            );
            if (!lastWs) return;
